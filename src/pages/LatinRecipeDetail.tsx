...
const LatinRecipeDetail = () => {
  ...
  const recipe = latinRecipes.find((r) => r.id === id);
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 border-b border-border/50 bg-background/95 backdrop-blur">
        <div className="mx-auto max-w-4xl flex h-14 items-center justify-between px-4">
          <Link to="/latin-kitchen" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="flex flex-wrap items-start gap-2 mb-2">
            ...
            {recipe.state && (
              <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                <MapPin className="h-3 w-3" /> {recipe.state}
              </span>
            )}
              ...
              <ol className="space-y-4">
                {recipe.instructions.map((step, i) => (
                  <li key={i} className="flex gap-3 text-sm leading-relaxed">
                    <span className="flex-shrink-0 w-6 h-6 rounded bg-brand-green/10 text-brand-green text-xs font-mono flex items-center justify-center">
                      {i + 1}
                    </span>
                    <span>{step}</span>
                  </li>
            ...
            <div className="mt-10 p-4 rounded-lg border border-border/50">
              ...
              <div className="grid grid-cols-5 gap-4 text-center">
                {[
                  ...
                  <div key={n.label}>
                    <p className="text-lg font-semibold text-brand-green">{n.value}<span className="text-xs text-muted-foreground">{n.unit}</span></p>
